...
function calculateTiltMatrix(e: MouseEvent, el: HTMLElement) {
  ...
  const maxRotation = 8;
  return {
    rotateX: -centerY * maxRotation,
    rotateY: centerX * maxRotation,
  };
}
...
export default function DataCard({ title, icon, children, delay = 0, className = '' }: DataCardProps) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      gsap.to(card, {
        duration: 0.4,
        rotateX: tiltMatrix.rotateX,
        rotateY: tiltMatrix.rotateY,
        ease: 'power2.out',
  ...
  return (
    <div
      ref={cardRef}
      className={`glass-card rounded-lg p-5 will-change-transform ${className}`}
